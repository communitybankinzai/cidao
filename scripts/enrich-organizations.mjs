// 印西の市民活動団体（WEB 公開情報からの取込で 1 行説明しかない）を
...
4. 取り込み元となった WEB 公開情報のページは元データなのでそれ "以外" の情報源を優先
...
async function enrichOne(org) {
  const regNote = org.inzai_registration_number
    ? `印西市市民活動推進条例 第10条登録済み（登録番号 ${org.inzai_registration_number}）`
    : '印西市未登録'
  const userPrompt =
    `団体名: ${org.name}\n` +
    `種別: ${org.type}（${regNote}）\n` +
